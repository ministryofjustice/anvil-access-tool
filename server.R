...
shinyServer(function(input, output, session) {
  ...
  form_data <- reactive({
    data <- data.table(
      ...
      bentham = max(as.integer(input$bentham),
                    responses_all()[id == tolower(input$quantum_id), bentham],
                    na.rm = T),
      bentham_status = bentham_status(),
      drug_convey = max(as.integer(input$drugs),
                        responses_all()[id == tolower(input$quantum_id), drug_convey],
                        na.rm = T),
      drug_convey_status = drug_convey_status(),
      network = max(as.integer(input$network),
                    responses_all()[id == tolower(input$quantum_id), network],
                    na.rm = T),
      network_status = network_status(),
      visitors = max(as.integer(input$visitors),
                     responses_all()[id == tolower(input$quantum_id), visitors],
                     na.rm = T),
      visitors_status = visitors_status(),
      novel_drugs = max(as.integer(input$novel_drugs),
                        responses_all()[id == tolower(input$quantum_id), novel_drugs],
                        na.rm = T),
